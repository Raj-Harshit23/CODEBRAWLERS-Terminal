...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def newalgo(self, game_state):
        ...
        if game_state.turn_number >= 2 and self.scored_on_locations:
            ...
            new_corner3 = self.prioritize_turret_positions(breach_location, new_corner3)
            new_corner4 = self.prioritize_turret_positions(breach_location, new_corner4)
            vleft_mid = self.prioritize_turret_positions(breach_location, vleft_mid)
            vright_mid = self.prioritize_turret_positions(breach_location, vright_mid)

            vleft_bottom = self.prioritize_turret_positions(breach_location, vleft_bottom)
            vright_bottom = self.prioritize_turret_positions(breach_location, vright_bottom)    

            vleft_midmid = self.prioritize_turret_positions(breach_location, vleft_midmid)
            vright_midmid = self.prioritize_turret_positions(breach_location, vright_midmid)

            new_turret_protect = self.prioritize_turret_positions(breach_location, new_turret_protect)

            bottom_2 = self.prioritize_turret_positions(breach_location, bottom_2)

            vleft_top = self.prioritize_turret_positions(breach_location, vleft_top)
            vright_top = self.prioritize_turret_positions(breach_location, vright_top)

            mid_2 = self.prioritize_turret_positions(breach_location, mid_2)

            top_2 = self.prioritize_turret_positions(breach_location, top_2)

            to_be_upgraded_max_prio = self.prioritize_turret_positions(breach_location, to_be_upgraded_max_prio)
            to_be_spawned_new = self.prioritize_turret_positions(breach_location, to_be_spawned_new)
